...
export class UsersService {
  ...
  async create(
    ...
    // Send initial credentials email in background
    this.mailService.sendAccountProvisionedEmail(saved.email, saved.firstName, tempPassword).catch(() => {});
  ...
  async findAll(
    ...
    if (query.search) {
      qb.andWhere('(user.firstName ILIKE :search OR user.lastName ILIKE :search OR user.email ILIKE :search OR CONCAT(user.firstName, \' \', user.lastName) ILIKE :search)', {
        search: `%${query.search}%`,
      });
    }
  ...
  async update(id: string, dto: UpdateUserDto, requesterRole?: string): Promise<Partial<User>> {
    ...
    if (roleChanged) {
      this.eventEmitter.emit('user.role_updated', {
        userId: saved.id,
        newRole: saved.role,
        oldRole: oldRole,
        updaterId: requesterRole === UserRole.SUPER_ADMIN ? 'System Root' : 'Admin',
      });
    }
  ...
  async updatePermissions(id: string, dto: UpdatePermissionsDto): Promise<Partial<User>> {
    ...
    const granted = dto.permissions.filter(p => !oldPermissions.includes(p));
    const revoked = oldPermissions.filter(p => !dto.permissions.includes(p));

    this.eventEmitter.emit('user.permissions_updated', {
      userId: saved.id,
      granted,
      revoked,
      updaterId: 'Super Admin', // Permissions endpoint currently restricted to Super Admin in controller
    });
